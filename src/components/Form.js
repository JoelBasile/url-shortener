...
class Form extends React.Component {
    ...
    onSubmit = async(event) => {
        ...
        // If user has inout a prefered alias then we use it, if not, we generate one
        var generatedKey;
        var generatedURL;
        ...
        if (this.state.preferedAlias !== '') {
            ...
            generatedKey = nanoid(6);
        }
        generatedURL = "MrCrabsta.github.io/" + generatedKey;

        // Sends infomation to database 
    ...
    validateInput = async () => {
        ...
        // Validate preferedAlias
        if (this.state.preferedAlias.length !== '') {
            // Checks for spaces
            if (this.state.preferedAlias.indexOf(' ') >= 0) {
